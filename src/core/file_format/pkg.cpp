...
bool PKG::Extract(const std::filesystem::path& filepath, const std::filesystem::path& extract,
    ...
    for (int i = 0; i < n_files; i++) {
        ...
        std::filesystem::path target_base = extract_path;
        std::string extract_path_str = extract_path.string();
        if (!extract_path_str.ends_with("-UPDATE")) {
            target_base /= title_id;
        }
        const auto filepath = target_base / "sce_sys" / name;
        std::filesystem::create_directories(filepath.parent_path());

        if (name.empty()) {
            // Just print with id
            Common::FS::IOFile out(target_base / "sce_sys" / std::to_string(entry.id),
        ...
        Common::FS::IOFile out(target_base / "sce_sys" / name, Common::FS::FileAccessMode::Write);
        if (!file.Seek(entry.offset)) {
            failreason = "Failed to seek to PKG entry offset";
            return false;
        }
        ...
        if (entry.id == 0x400 || entry.id == 0x401 || entry.id == 0x402 ||
            ...
            // Pad to multiple of AES block size (16) to prevent buffer overflow
            size_t padded_size = ((entry.size + 15) / 16) * 16;
            decNp.resize(padded_size);
            if (!file.Seek(entry.offset)) {
                failreason = "Failed to seek to PKG entry offset";
                return false;
            }
            ...
            Common::FS::IOFile out(target_base / "sce_sys" / name,
                                   Common::FS::FileAccessMode::Write);
        ...
        if (uroot_reached) {
            for (int i = 0; i < 0x10000; i += ent_size) {
                ...
                if (dirent.ino != 0) {
                    ...
                    std::string extract_path_str = extract_path.string();
                    if (!extract_path_str.ends_with("-UPDATE")) {
                        extractPaths[ndinode_counter] = extract_path / title_id;
                    } else {
